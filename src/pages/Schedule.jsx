...
export default function Schedule() {
  ...
  const [events, setEvents] = useState([
    {
      ...
      notes: 'Focus on back & shoulders',
    },
    {
      id: 2,
      title: 'HIIT Class',
      client: 'Group (8 people)',
      day: 2, // Tuesday
      time: '7:30 AM',
      duration: '45 min',
      type: 'class',
      color: 'from-indigo-500 to-violet-600',
      notes: 'Burn 500+ calories',
    },
    {
      id: 3,
      title: 'Yoga & Mobility',
  ...
  const renderEventCell = (dayIndex, time) => {
    ...
    return dayEvents.map((event) => (
      <div
        key={event.id}
        className={`text-xs text-white p-2 rounded-lg mb-1 cursor-pointer shadow-md transform hover:scale-105 transition-all duration-200 bg-gradient-to-r ${event.color} transform-hover glow-hover`}
        onClick={(e) => {
          e.stopPropagation();
          openModal(event);
        }}
      ...
      >
        ...
        <div className="opacity-90 text-xs">{event.client}</div>
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      ...
      <div className="ml-64 flex-1 p-6 animate-fade-in">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Manage your sessions, classes, and availability</p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm p-6 rounded-3xl shadow-xl border border-white/30 mb-8 transform-hover">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-3">
              <button
                onClick={goToToday}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition font-medium"
              >
                Today
              </button>
              <button
                onClick={() => openModal()}
                className="px-5 py-2.5 bg-gradient-to-r from-blue-500 to-emerald-600 hover:from-emerald-600 hover:to-green-600 text-white rounded-xl font-medium transform-hover shadow-md hover:shadow-lg"
              >
                ➕ Add Session
              </button>
            </div>
              ...
              >
                ...
                <div
                  className={`mt-1 w-10 h-10 mx-auto rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                    isToday(date)
                      ? 'bg-white/30'
                      : 'bg-gradient-to-br from-gray-100 to-gray-200 text-gray-800 hover:from-blue-500/20'
                  }`}
                >
                  {date.getDate()}
                </div>
              ...
              <div key={time} className="grid grid-cols-8 text-sm">
                ...
                {weekDates.map((date, dayIndex) => (
                  <div
                    ...
                    onClick={() => {
                      setNewEvent({ ...newEvent, day: dayIndex, time });
                      openModal();
                    }}
                  >
                    {/* Hover hint */}
        ...
        <div className="mt-8 bg-white/90 backdrop-blur-sm p-6 rounded-3xl shadow-xl border border-white/30 text-sm text-gray-600">
          <div className="flex flex-wrap items-center gap-6">
            ...
            {[
              { type: 'Session', color: 'from-blue-500 to-emerald-600' },
              { type: 'Class', color: 'from-indigo-500 to-violet-600' },
              ...
              <form className="space-y-5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleEventChange}
                    rows="2"
                    placeholder="E.g. Focus on back & shoulders"
                    className="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-3 focus:ring-blue-500/50"
                  ></textarea>
                </div>
